...
#### Note: "reset_db" accepts two optional parameters, that represent the count of the categories to be created in addition to the Root Category and the number of attempts to create unique similarities.

reset_db [-c--categories < int > [-s--similarities < int >]]

For example: **py manage.py reset_db -c 10** or **py manage.py reset_db -c 10 -s 10**

#### Note: to run the tests use the following command
